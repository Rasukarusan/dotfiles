...
- **解説の書き直しを依頼できる**: 概要・グループの意図・セクション解説それぞれに「書き直しを依頼」ボタン。Claudeが `review-data.json` を書き直すと、**画面が自動で新しい解説に入れ替わる**(コメントは保持。入力中は「画面に反映する」バーが出る)
