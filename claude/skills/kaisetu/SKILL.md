...
→ ⑤ 人間が画面でコメント(diff行・グループ・概要やAI解説にも) → 「レビュー完了」
...
- **tagline と overview の各行は「エンジニアに分かる説明 ＝ 非エンジニアにも伝わる結果」の形で書く**(＝は全角)。
  右側は「その結果何がどうなるのか」を、コードを読まない人に分かる言葉で書く(関数名・ファイル名は使わない)。
  例: `URL生成とホスト判定を resolveAppUrl に集約 ＝ URLの作り方が1か所になり、直し忘れによるリンク切れがなくなる`
  書き方の詳細は schema.md「概要の書き方」を参照。
...
2. まとめをユーザーに提示し、コメントに対応する(修正 or 回答)。
   ...
   - `comments` = diff行への指摘。修正するか、根拠を答える。
   - `docComments` / `groupComments` = **私が書いた解説文(概要・グループの意図・セクション解説)への
     コメント**であることが多い。「分かりにくい」「書き直して」なら
     `$REVIEW_DIR/review-data.json` の該当フィールドを書き直して保存する。
     サーバがファイルを読み直すので、**画面は数秒で自動的に新しい解説へ入れ替わる**(コメントは保持される)。
     どのフィールドを直すかは schema.md「解説文へのコメント」の表を見る。
     `groups[].id` / `sections[].id` / `hunks` の構成は変えない(コメントの位置がずれる)。
     コード自体への指摘なら、通常どおり修正 or 回答する。
