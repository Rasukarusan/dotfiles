...
alias co='git checkout $(git branch -a | tr -d " " |fzf-tmux -p80% --prompt "CHECKOUT BRANCH>" --preview "git log --color=always {}" --bind '\''ctrl-y:execute-silent(echo -n {} | sed -e "s/^\*//" | perl -pe "s|remotes/origin/||" | tr -d "\n" | pbcopy)'\'' | head -n 1 | sed -e "s/^\*\s*//g" | perl -pe "s/remotes\/origin\///g")'
...
alias codex='codex --sandbox workspace-write --ask-for-approval on-request --config approvals_reviewer="auto_review"'
